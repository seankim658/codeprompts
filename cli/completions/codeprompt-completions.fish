...
function __fish_codeprompt_global_optspecs
	string join \n profile= list-profiles write-profile= delete-profile= force include= exclude= exclude-priority exclude-from-tree literal-brackets gitignore d/diff-staged u/diff-unstaged no-tokens c/encoding= o/output= l/no-line-numbers no-codeblock absolute-paths no-clipboard t/template= no-spinner json issue= verbose no-warnings h/help V/version
end
