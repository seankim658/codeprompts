...
Register-ArgumentCompleter -Native -CommandName 'codeprompt' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'codeprompt' {
            [CompletionResult]::new('--profile', '--profile', [CompletionResultType]::ParameterName, 'Run using a saved profile from the project-local `.codeprompt.toml`')
            [CompletionResult]::new('--write-profile', '--write-profile', [CompletionResultType]::ParameterName, 'Save the flags from this run as a profile, then exit. Use `--write-profile NAME` to name it, or bare `--write-profile` to be prompted')
            [CompletionResult]::new('--delete-profile', '--delete-profile', [CompletionResultType]::ParameterName, 'Delete a saved profile from the project-local `.codeprompt.toml`')
            [CompletionResult]::new('--show-profile', '--show-profile', [CompletionResultType]::ParameterName, 'Show a saved profile''s values and the command it would run')
